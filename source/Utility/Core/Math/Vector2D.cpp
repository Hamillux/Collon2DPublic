...
#include <cfenv>
#include <DxLib.h>
#include "Utility/Core/Rendering/ScreenParams.h"
...
Vector2D Vector2D::WorldToViewport(const Vector2D& world_position, const ScreenParams& screen_params)
{
	return screen_params.GetMatrixWorldToViewport().TransformVector(world_position);
}

Vector2D Vector2D::ViewportToWorld(const Vector2D& screen_position, const ScreenParams& screen_params)
{
	return screen_params.GetMatrixViewportToWorld().TransformVector(screen_position);
}
