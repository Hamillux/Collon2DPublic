...
public:
	...
	// ワールド座標をビューポート座標に変換
	static Vector2D WorldToViewport(const Vector2D& world_position,const ScreenParams& viewport_params);
	// ビューポート座標をワールド座標に変換
	static Vector2D ViewportToWorld(const Vector2D& viewport_position,const ScreenParams& screen_params);
