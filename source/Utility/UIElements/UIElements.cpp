...
{
	int left, top;
	left_top.ToInt(left, top);

	int extent_x, extent_y;
	extent.ToInt(extent_x, extent_y);
...
void UIElements::ImageRectangle::Draw(const Vector2D& left_top, const Vector2D& extent) const
{
	...
	CalcRectCenter(left_top, extent).ToInt(draw_center_x, draw_center_y);
	...
	, horizontal_alignment(horizontal_alignment_in)
{
	...
	text_extent.x = GetDrawStringWidth(_T(text.c_str()), strlenDx(text.c_str()));
...
{
	const TCHAR* text_T = _T(text.c_str());
	const int text_left = CalcTextLeft(rect_left_top, rect_extent);
	const int text_top = CalcTextTop(rect_left_top, rect_extent);
	
	if (alpha == DrawBlendInfo::MAX_BLEND_VALUE)
	{
		DrawStringHelper::DrawString(font_size, text_left, text_top, _T(text.c_str()), fill_color, border_color);
		return;
	}
