...
SceneBase::SceneBase()
	...
	, bg_area_right_bottom(FLT_MAX, WINDOW_SIZE_Y)
	, _world_timer(0.f)
...
{
	_screen_params.screen_scale = 1.0f;
...
SceneType SceneBase::Tick(float delta_seconds)
{
	...
	// スクリーンパラメータはアクターの位置などに依存する可能性があるため
	// アクターの更新後にスクリーンパラメータの更新を行う
	UpdateScreenParams(delta_seconds);
...
void SceneBase::Draw()
{
	...
	{
		if (
			actor->ShouldCallDraw() && (actor->GetShouldDrawAlways() || actor->IsInDrawArea(_screen_params) && !actor->IsHidden())
			)
		{
			actor->Draw(_screen_params);
		}
	}

	// 2. パーティクル描画
	ParticleManager::GetInstance().Draw(_screen_params);
...
void SceneBase::Finalize()
{
	...
	_actors_to_add.shrink_to_fit();

	_game_speed_rate = 1.0f;
}

void SceneBase::UpdateScreenParams(const float delta_seconds)
{
}
...
void SceneBase::DrawDebugLine(const Vector2D& start_world, const Vector2D& end_world, const int line_color, const int line_thickness, const DrawBlendInfo& blend_info)
{
	...
	const Vector2D start_screen = Vector2D::WorldToViewport(start_world, _screen_params);
	const Vector2D end_screen = Vector2D::WorldToViewport(end_world, _screen_params);
...
void SceneBase::DrawDebugRect(const FRect& rect, const int line_color, const int line_thickness, const DrawBlendInfo& blend_info)
{
	...
	{
		const Vector2D start = Vector2D::WorldToViewport(vertices[i], _screen_params);
		const Vector2D end = Vector2D::WorldToViewport(vertices[(i + 1) % 4], _screen_params);
...
Actor* SceneBase::CreateAndInitializeActorByEntityType(const EEntityType entity_type, const ActorInitialParams* actor_params)
{
	...
	{
		_actors.push_back(created_actor);
	}
...
void SceneBase::DestroyActor(Actor*& destroyee)
{
	...
	if (IsContainedInActors(destroyee, &it_destroyee))
	{
...
bool SceneBase::IsContainedInActors(Actor* actor, std::vector<Actor*>::iterator* outIt)
{
	auto it = std::find(_actors.begin(), _actors.end(), actor);
	if (outIt != nullptr)
	{
...
void SceneBase::RemoveActor(Actor* actorToRemove)
{
	if (IsContainedInActors(actorToRemove))
	{
		_actors_to_remove.push_back(actorToRemove);
	}
}

void SceneBase::AddActor(Actor* actorToAdd)
{
	if (!actorToAdd->IsInitialized())
	{
		throw std::runtime_error("Attempted to add uninitialized actor");
	}

	if(!IsContainedInActors(actorToAdd))
	{
		_actors_to_add.push_back(actorToAdd);
	}
}
...
void SceneBase::AddBackgroundToLayer(BackgroundParams bg_params)
{
	...
	{
		if (bg_params.distance >= iterator->distance)
		{
...
void SceneBase::DrawBackground() const
{
	...
	for (const auto& bg_params : _bg_layer)
	{
		...
		// TODO: screen_paramsから取得する
		const float cz = _screen_params.GetCameraZ();
		const float d = bg_params.distance;
		const float ext_stage_y = UNIT_TILE_SIZE * 18 * 2;
		const float ext_screen_y = UNIT_TILE_SIZE * 18;
		const float ext_screen_x = UNIT_TILE_SIZE * 32;
		const float ext_bg_y = ext_stage_y + ext_screen_y * (d - 1.f);
		const float ext_bg_x = ext_bg_y * bg_width_per_height;

		const float cx = _screen_params.world_offset.x;
		const float cy = _screen_params.world_offset.y;

		const float top = 
			( -(ext_stage_y / ext_screen_y + d - 1.f) - 2.f * cy / ext_screen_y ) / 
			(d - _screen_params.GetCameraZ());
		const float bottom = 
			( (ext_stage_y / ext_screen_y + d - 1.f) - 2.f * cy / ext_screen_y) /
			(d - _screen_params.GetCameraZ());

		// 誤差を考慮し, 理論値より左右に1枚ずつ余分に描画する
		const int i_x_start = floor((cx - (cz-1)*ext_screen_x/2.f) / ext_bg_x) - 1;
		const int i_x_end = ceil((cx + (2 * d - 1 - cz) * ext_screen_x / 2.f) / ext_bg_x) + 1;
		const auto get_left = [&](const int i) -> float
			{
				float ret =
					(i * ext_bg_x - ext_screen_x / 2.f * (d - 1.f) - cx) /
					((d - cz) * ext_screen_x / 2.f);
				return ret;
			};

		for (int i = i_x_start; i < i_x_end; i++)
		{
			const float left = get_left(i);
			const float right = get_left(i + 1);
			const Vector2D left_top_viewport = _screen_params.TransformPosition_ScreenToViewport(Vector2D{ left,top });
			const Vector2D right_bottom_viewport = _screen_params.TransformPosition_ScreenToViewport(Vector2D{ right,bottom });
			DxLib::DrawExtendGraph(
				left_top_viewport.x,
				left_top_viewport.y,
...
int SceneBase::GetBGDrawAreaLeft() const
{
	return bg_area_left_top.x;
}

void SceneBase::SetBGDrawArea(const Vector2D& left_top, const Vector2D& right_bottom)
{
	bg_area_left_top = left_top;
	bg_area_right_bottom = right_bottom;
}

int SceneBase::GetBGDrawAreaRight() const
{
	if (bg_area_right_bottom.x == FLT_MAX)
	{
		return Vector2D::ViewportToWorld(Vector2D(WINDOW_SIZE_X, 0), _screen_params).x;
	}
	
	return bg_area_right_bottom.x;
}
...
void SceneBase::DrawWorldGrid(const int color, const uint8_t alpha) const
{
	...
	// 背景の描画エリアを設定
	const Vector2D bg_area_left_top_viewport = Vector2D::WorldToViewport(bg_area_left_top, _screen_params);
	Vector2D bg_area_right_bottom_viewport = Vector2D::WorldToViewport(bg_area_right_bottom, _screen_params);
	if (bg_area_right_bottom.x == FLT_MAX)
	{
	...
	const Vector2D& screen_world_offset = _screen_params.world_offset;
	...
	Vector2D::WorldToViewport(Vector2D(left, top), _screen_params).ToInt(s_left, s_top);
	Vector2D::WorldToViewport(Vector2D(right, bottom), _screen_params).ToInt(s_right, s_bottom);

	const uint16_t scaled_unit_tile_size =
		Vector2D::WorldToViewport(Vector2D{ UNIT_TILE_SIZE,0 }, _screen_params).x -
		Vector2D::WorldToViewport(Vector2D{ 0,0 }, _screen_params).x;
	while (s_left > 0)
	{
