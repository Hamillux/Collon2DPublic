...
void InGameSceneState_GameOver::DrawForeground(ParentSceneClass& parent_scene, const CanvasInfo& canvas_info)
{
	...
	if (_timer >= fade_end_time)
	{
		...
		DrawStringHelper::DrawStringC(GAME_OVER_FONT_SIZE, x, y, _T(u8"GAME OVER"), GetColor(255, 255, 255));
		...
		DrawStringHelper::DrawStringC(CAUSE_OF_DEATH_FONT_SIZE, x, cause_of_death_y, _T(cause_of_death.c_str()), GetColor(255, 255, 255));
		...
		for (int i = 0; i < 3; ++i) 
		{
			...
			{
			case BUTTON_RETRY:
				DrawStringHelper::DrawStringC(BUTTON_FONT_SIZE, x, buttons_middle_y - OFFSET, _T(u8"RETRY"), color);
				break;
			case BUTTON_RETURN_TO_STAGE_SELECT:
				DrawStringHelper::DrawStringC(BUTTON_FONT_SIZE, x, buttons_middle_y, _T(u8"RETURN TO STAGE SELECT"), color);
				break;
			case BUTTON_RETURN_TO_TITLE:
				DrawStringHelper::DrawStringC(BUTTON_FONT_SIZE, x, buttons_middle_y + OFFSET, _T(u8"RETURN TO TITLE"), color);
				break;
			}
		}
	}
