...
void InGameSceneState_Playing::DrawForeground(ParentSceneClass& parent_scene, const CanvasInfo& canvas_info)
{
	...
	// 残り時間
	char time_string[32];
	sprintf_s(time_string, "%03.f", parent_scene._remaining_time);
	...
	// ステージ名
	const std::string stage_name = parent_scene.GetStageRef().GetStageName();
	...
	SetFontSize(24);
	DrawFormatString(score_left, score_top, 0xFFFFFF, "%014d", parent_scene._total_score);
	SetFontSize(DEFAULT_FONT_SIZE);

	if (_reset_timer > 0.f)
	{
		// 画面全体を暗くする
		BlendDrawHelper::DrawBox(DrawBlendInfo(DX_BLENDMODE_ALPHA, 128), 0, 0, canvas_info.width, canvas_info.height, DxLib::GetColor(0, 0, 0), TRUE);

		// 「このステージをやり直します...」
		constexpr const TCHAR* text = u8"このステージをやり直します...";
		const int text_width = DxLib::GetDrawStringWidth(text, strlenDx(text));
		...
		DrawString(text_left, text_top, text, text_color);
...
void InGameSceneState_Playing::UpdateScreenParams(ParentSceneClass& parent_scene, const float delta_seconds)
{
	...
	ScreenParams& screen_params = parent_scene._screen_params;

	// X方向
	{
		// screen_world_offsetはX方向にプレイヤーの1.3倍の速度で移動する
		screen_params.world_offset.x += delta_player_position.x * 1.5f;

		const Vector2D player_pos_screen = 
			screen_params.TransformPosition_WorldToScreen(player_world_pos);
		...
		{
			screen_params.ChangeWorldOffset_World_Screen(player_world_pos, Vector2D{ left_limit, player_screen_y });
		}
		else if (player_screen_x > right_limit)
		{
			screen_params.ChangeWorldOffset_World_Screen(player_world_pos, Vector2D{ right_limit, player_screen_y });
		}
	}

	// Y方向
	{
		constexpr float top_limit = -1 + 1.f / 5.f;
		constexpr float bottom_limit = 1 - 1.f / 5.f;
		const Vector2D player_pos_screen = screen_params.TransformPosition_WorldToScreen(parent_scene.GetPlayerRef()->GetActorWorldPosition());
		const float player_screen_x = player_pos_screen.x;
		const float player_screen_y = player_pos_screen.y;
		if (player_screen_y < top_limit)
		{
			screen_params.ChangeWorldOffset_World_Screen(player_world_pos, Vector2D{ player_screen_x, top_limit});
		}
		else if (player_screen_y > bottom_limit)
		{
			screen_params.ChangeWorldOffset_World_Screen(player_world_pos, Vector2D{ player_screen_x, bottom_limit });
		}
	}
}
