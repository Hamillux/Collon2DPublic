...
#include "SceneObject/Actor/AllActorsInclude_generated.h"
...
void StageInteractiveScene::Initialize(const SceneBaseInitialParams* const scene_params)
{
	...
	_stage->stage_events.OnStageSizeChanged.Bind([this]() 
		{
			SetBGDrawArea(_stage->GetStageLeftTop(), _stage->GetStageRightBottom());
		});
}
...
{
	SetBGDrawArea(stage.GetStageLeftTop(), stage.GetStageRightBottom());
...
bool StageInteractiveScene::ExistsSpawnActorInfo(Actor* actor) const
{
	return actor_spawn_info_map.find(actor) != actor_spawn_info_map.end();
}

void StageInteractiveScene::ClampScreenWorldOffsetInStage(ScreenParams& screen_params) const
{
	...
	const Vector2D screen_half_extent = screen_params.GetScreenHalfExtentScaled();

	const float cx_min = stage_left_top.x + screen_half_extent.x;
	const float cx_max = stage_right_bottom.x - screen_half_extent.x;
	const float cy_min = stage_left_top.y + screen_half_extent.y;
	const float cy_max = stage_right_bottom.y - screen_half_extent.y;
	screen_params.world_offset.x = Clamp(screen_params.world_offset.x, cx_min, cx_max);
	screen_params.world_offset.y = Clamp(screen_params.world_offset.y, cy_min, cy_max);
}

float StageInteractiveScene::GetMaxScreenScale() const
{
	return _screen_params.GetMaxScale(GetStageRef().GetStageSize());
}
...
void StageInteractiveScene::GetActorOccupyingArea(Actor* const actor, int& tile_index_left, int& tile_index_top, int& tile_index_right, int& tile_index_bottom)
{
	...
	actor->GetOccupyingTileCounts(num_tiles_x, num_tiles_y, snap_pos_to_actor_pos);
