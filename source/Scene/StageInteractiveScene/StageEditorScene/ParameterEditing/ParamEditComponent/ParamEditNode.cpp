...
#include "SceneObject/Actor/Mapchip/Block/BlockTexturing.h"
...
{
	ImGui::Texture icon_texture;
	MasterHelper::GetGameIconImguiImage(MdItem::Get(node.value).icon_id, icon_texture);
	...
	{
		const char* item_name = MdItem::Get(node.value).item_name.c_str();
	...
	{
		// 値編集開始
		node.OnBeginValueChange();
		ImGui::OpenPopup(popup_str_id);
	}

	ValueEditModalResult result = ShowItemSelectionDialog(node.value, popup_str_id, is_popup_open_timing);
	switch (result)
	{
	case ValueEditModalResult::CONFIRMED:
		node.OnEndValueChange();
		break;
	case ValueEditModalResult::CANCELED:
		node.value = *node.pLastValue;
		node.OnEndValueChange();
		break;
	default:
		break;
