...
void StageEditorSceneState_Edit_Modify::DrawForeground(ParentSceneClass& parent_scene, const CanvasInfo& canvas_info)
{
	...
	if (actor_modification_context != nullptr)
	{
...
void StageEditorSceneState_Edit_Modify::HandleInput(ParentSceneClass& parent_scene, const float delta_seconds)
{
	...
	if (DeviceInput::IsPressed(MOUSE_INPUT_LEFT, DeviceInput::Device::MOUSE) && is_mouse_on_stage)
	{
		...
		{
			SelectModificationTargetActor(parent_scene, hovered_actor);
		...
		{
			UnSelectModificationTargetActor();
		}
	}
...
void StageEditorSceneState_Edit_Modify::OnUndo(StageEditorScene& parent_editor_scene)
{
	...
	if(actor_modification_context)
	{
		SelectModificationTargetActor(parent_editor_scene, actor_modification_context->actor);
	}
}
...
void StageEditorSceneState_Edit_Modify::SelectModificationTargetActor(StageEditorScene& parent_scene, Actor* target_actor)
{
...
void StageEditorSceneState_Edit_Modify::UnSelectModificationTargetActor()
{
	if (actor_modification_context == nullptr)
	{
		return;
