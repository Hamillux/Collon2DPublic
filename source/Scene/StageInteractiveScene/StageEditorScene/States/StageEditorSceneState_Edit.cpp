...
void StageEditorSceneState_Edit::ShowRightbarHead(StageEditorScene& parent_editor_scene)
{
	...
	ImGui::BeginAlignedChild("radiobuttons_editmode", { 0.5, 0 }, { 0.5, 0 }, group_size, { 0, 32 });
	{
		...
		ImGui::BeginGroup();
		{
			...
			for (size_t i = 0; i < edit_modes.size(); i ++)
			{
				const ImGui::Texture& img = images.at(i);
				ImGui::PushID(i);
				ImGui::ImageRadioButton("##radio_editmode", selected_edit_mode, static_cast<int>(edit_modes.at(i)), img.im_tex_id, button_size, img.uv0, img.uv1, StageEditorColor::BackColor(1));
				ImGui::PopID();
				if (i < edit_modes.size() - 1)
				{
					ImGui::SameLine();
				}
			}
...
std::shared_ptr<SceneState<StageEditorScene>> StageEditorSceneState_Edit::MakeSubState(const EditMode edit_mode)
{
	...
	//case EditMode::REMOVE:
		//return std::make_shared<StageEditorSceneState_Edit_Remove>();
	}
...
{
	for (int i = 0; i < edit_modes.size(); i++)
	{
		if (current_edit_mode == edit_modes.at(i))
		{
			return edit_modes.at((i + 1) % edit_modes.size());
		}
	}
}
...
StageEditorSceneState_Edit::EditMode StageEditorSceneState_Edit::GetPrevEditMode(const EditMode current_edit_mode)
{
	for (int i = 0; i < edit_modes.size(); i++)
	{
		if (current_edit_mode == edit_modes.at(i))
		{
			return edit_modes.at((i + edit_modes.size() - 1) % edit_modes.size());
		}
	}
}
...
void StageEditorSceneState_Edit::UpdateScreenParams(ParentSceneClass& parent_scene, const float delta_seconds)
{
	...
	// キーボード操作
	{
		...
		{
			const float camera_speed = 1000.f * screen_params.screen_scale;
			...
			screen_params.world_offset += input_dir * camera_speed * delta_seconds * multiplier;
		}
	}
	...
	{
		const float HALF_SCREEN_WIDTH = screen_params.GetScreenHalfExtentScaled().x;
		const float  HALF_SCREEN_HEIGHT = screen_params.GetScreenHalfExtentScaled().y;
		...
		screen_params.world_offset -= Vector2D
		(
	...
	if (parent_scene.IsMouseOnStage(mouse_pos))
	{
		...
		if (!DeviceInput::WheelIsStopped())
		{
			...
			int n = -2;
			while (powf(2.f, n) < screen_params.screen_scale)
			{
				n++;
			}
			...
			if (screen_params.screen_scale != new_scale && clamp(new_scale, MIN_SCALE, max_scale) == new_scale)
			{
				screen_params.ChangeScale(new_scale, DeviceInput::GetMousePosition());
			}
		}
		...
		{
			screen_params.ChangeScale(1.f, GetWindowSize() * 0.5f);
		}
	}
}
