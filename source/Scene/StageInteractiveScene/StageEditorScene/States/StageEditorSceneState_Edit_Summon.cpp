...
void StageEditorSceneState_Edit_Summon::Draw(ParentSceneClass& parent_scene)
{
	...
	{
		_created_actor_to_summon->Draw(parent_scene._screen_params);
		...
		const Vector2D center = parent_scene._screen_params.TransformPosition_WorldToViewport(_created_actor_to_summon->GetActorWorldPosition());
