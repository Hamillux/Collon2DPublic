...
void StageEditorSceneState_Edit_Move::Draw(ParentSceneClass& parent_scene)
{
	...
	if (_moving_info || _duplication_info)
	{
		...
		{
			actor->Draw(parent_scene._screen_params);
			parent_scene.DrawActorConvex(actor, DxLib::GetColor(convex_r, convex_g, convex_b));
		}
	}
	...
	if (_current_substate == SubState::RectSelecting)
	{
		...
		BlendDrawHelper::DrawBox(
			DrawBlendInfo(DX_BLENDMODE_ADD, rect_a),
...
void StageEditorSceneState_Edit_Move::UpdateScreenParams(ParentSceneClass& parent_scene, const float delta_seconds)
{
	...
	case SubState::MovingDuplicatedActors:
		__super::UpdateScreenParams(parent_scene, delta_seconds);
		return;
	}
}
...
void StageEditorSceneState_Edit_Move::HandleInputImpl_MovingActors(StageEditorScene& parent_scene, const float delta_seconds)
{
	...
	const Vector2D mouse_pos_world = parent_scene._screen_params.TransformPosition_ViewportToWorld(mouse_pos);
	...
	else if (!DeviceInput::IsActive(MOUSE_INPUT_LEFT, DeviceInput::Device::MOUSE))
	{
		// 移動完了
		int mouse_tile_index_x, mouse_tile_index_y;
		GetTileIndex(parent_scene._screen_params.TransformPosition_ViewportToWorld(mouse_pos), mouse_tile_index_x, mouse_tile_index_y);
...
void StageEditorSceneState_Edit_Move::HandleInputImpl_RectSelecting(StageEditorScene& parent_scene, const float delta_seconds)
{
	if (_drag_drop_end_viewport_pos != nullptr)
	{
		...
		const FRectAA rect{
			parent_scene._screen_params.TransformPosition_ViewportToWorld(Vector2D{ rect_left_viewport, rect_top_viewport }),
			parent_scene._screen_params.TransformPosition_ViewportToWorld(Vector2D{ rect_right_viewport, rect_bottom_viewport })
		};
...
void StageEditorSceneState_Edit_Move::HandleInputImpl_MovingDuplicatedActors(StageEditorScene& parent_scene, const float delta_seconds)
{
	if (_drag_drop_end_viewport_pos)
	{
		...
		GetTileIndex(
			parent_scene._screen_params.TransformPosition_ViewportToWorld(*_drag_drop_end_viewport_pos),
			mouse_tile_to_x,
			mouse_tile_to_y
		);
	...
	for (auto& actor : _duplication_info->duplicated_actors)
	{
		int mouse_tile_index_x, mouse_tile_index_y;
		GetTileIndex(
			parent_scene._screen_params.TransformPosition_ViewportToWorld(DeviceInput::GetMousePosition()),
			mouse_tile_index_x,
			mouse_tile_index_y
		);
...
void StageEditorSceneState_Edit_Move::BeginMovingSelectedActors(StageEditorScene& parent_scene)
{
	...
	int mouse_tile_index_x, mouse_tile_index_y;
	GetTileIndex(
		parent_scene._screen_params.TransformPosition_ViewportToWorld(mouse_pos),
		mouse_tile_index_x,
		mouse_tile_index_y
	);
