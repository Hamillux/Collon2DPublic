...
void StageEditorSceneState_ThumbnailShooting::OnEnterState(ParentSceneClass& parent_scene)
{
	...
	// スクリーンの拡大率を1.0に設定
	parent_scene._screen_params.ChangeScale(1.f, GetWindowHalfSize());
}
...
void StageEditorSceneState_ThumbnailShooting::UpdateScreenParams(ParentSceneClass& parent_scene, const float delta_seconds)
{
	switch (_current_substate)
	{
	case ThunmailShootingSubState::Default:
	{
		...
		parent_scene._screen_params.world_offset += move_dir * move_speed * delta_seconds;

		// 上下左右キーで1ずつ移動
		if (DeviceInput::IsPressed(KEY_INPUT_UP))
		{
			parent_scene._screen_params.world_offset.y -= 1;
		}
		...
		{
			parent_scene._screen_params.world_offset.x -= 1;
		}
		...
		// スクリーンオフセットを整数値にする
		parent_scene._screen_params.world_offset.x = static_cast<int>(parent_scene._screen_params.world_offset.x);
		parent_scene._screen_params.world_offset.y = static_cast<int>(parent_scene._screen_params.world_offset.y);
	}
	break;
	case ThunmailShootingSubState::ReleasingShutter:
	case ThunmailShootingSubState::Confirm:
...
void StageEditorSceneState_ThumbnailShooting::UpdateStageThumbnail(const Stage& stage) const
{
	...
	_capture_result->SaveToPNG(file_path.c_str());
}
