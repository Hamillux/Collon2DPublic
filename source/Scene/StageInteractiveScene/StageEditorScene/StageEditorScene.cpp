...
Actor* StageEditorScene::GetActorAt(const Vector2D& viewport_position) const
{
	...
	const Vector2D world_position = _screen_params.TransformPosition_ViewportToWorld(viewport_position);
...
void StageEditorScene::DrawActorConvex(Actor* actor, int color, int alpha)
{
	...
	for (auto& v : vertices)
	{
		v = Vector2D::WorldToViewport(v, _screen_params);
	}
...
void StageEditorScene::UpdateScreenParams(const float delta_seconds)
{
	_state_stack->UpdateScreenParams(*this, delta_seconds);

	_screen_params.screen_scale = std::min(GetMaxScreenScale(), _screen_params.screen_scale);
	ClampScreenWorldOffsetInStage(_screen_params);
}
...
{
	// アクターがステージ外に出ていないかチェックし, 必要に応じてnew_commandにサブコマンドを追加する
	...
	// A. 完全に外に出ている場合, そのアクターを削除する(CmdRemoveActor)
	// B. 一部だけ外に出ている場合, そのアクターをステージ内に戻す
	std::vector<Actor*> remove_list;
	std::vector<Actor*> move_list;

	for (auto& actor : _actors)
	{
		if (!ExistsSpawnActorInfo(actor))
		{
			continue;
		}
		...
		{
			throw std::runtime_error("Actor is too large");
		}
