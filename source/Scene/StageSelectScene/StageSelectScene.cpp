...
namespace
{
	...
	void GetPageAndCell(const int cell_index, int& page, int& cell_x, int& cell_y)
	{
		page = cell_index / SmallThumbNailsGrid::num_cells_total;
		const int i = cell_index % SmallThumbNailsGrid::num_cells_total;
		cell_x = i % SmallThumbNailsGrid::num_cells_x;
		cell_y = i / SmallThumbNailsGrid::num_cells_x;
	}
}
...
void StageSelectScene::DrawForeground(const CanvasInfo& canvas_info)
{
	...
	DrawStringHelper::DrawStringC(64, _mode_str_center.x, _mode_str_center.y, mode_str, mode_str_fill_color);
	...
	if (GetSelectedStage() != nullptr)
	{
		...
		// ステージ名
		DrawStringHelper::DrawStringC(32, CENTER_X, TOP + 12, _T(GetSelectedStage()->GetStageName().c_str()), FONT_COLOR);

		// 制限時間
		const std::string time_limit_str = u8"制限時間: " + std::to_string(GetSelectedStage()->GetTimeLimit()) + u8"秒";
		DrawStringHelper::DrawStringC(24, CENTER_X, TOP + 12 + 32, _T(time_limit_str.c_str()), FONT_COLOR);
		...
		const char* show_str = description_substr.empty() ? _T(u8"ステージの説明はありません") : _T(description_substr.c_str());
		DrawObtainsString_CharClip(desc_area.left, desc_area.top, 32, show_str, FONT_COLOR);
		...
		{
			DrawString(desc_area.left, 630, _T(u8"…"), 0xFFFFFF);
		}
	...
	// ページ番号表示
	const std::string page_str = std::to_string(_current_page + 1) + " / " + std::to_string(GetNumPages());
	DrawStringHelper::DrawStringC(32, current_page_str_center.x, current_page_str_center.y, _T(page_str.c_str()), 0xFFFFFF);
...
void StageSelectScene::ChangeStageSelectMode(const StageSelectMode new_mode)
{
	...
	// NOTE:「編集」では先頭セルが「新規作成」になるので, 
	// 「遊ぶ」「消す」との遷移時に選択ステージがずれないようにする
