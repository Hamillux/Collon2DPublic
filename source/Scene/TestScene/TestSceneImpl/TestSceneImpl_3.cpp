...
void TestSceneImpl_3::DrawForeground(const CanvasInfo& canvas_info)
{
	...
	SetDrawArea(0, 400, 200, 500);
	constexpr const TCHAR* TEXT = _T(u8"１テキストテキス２テキストテキス３テキ\n\nストテキス４テキストテキス５テキストテキスト６テキストテキスト７テキストテキスト８テキストテキスト");
	int x, y;
	int ret = GetObtainsStringCharPosition_CharClip(0, 0, 20, TEXT, DxLib::strlenDx(TEXT), &x, &y);
	DrawObtainsString_CharClip(0, 0, 20, TEXT, 0xFFFFFF);
	SetDrawArea(last_draw_area.left, last_draw_area.top, last_draw_area.right, last_draw_area.bottom);


	std::string ret_str = std::string("Ret==") + std::to_string(ret);
	DrawString(500, 0, _T(ret_str.c_str()), 0xFFFFFF);
	DrawFormatString(500, 20, 0xFFFFFF, _T("x=%d, y=%d"), x, y);
