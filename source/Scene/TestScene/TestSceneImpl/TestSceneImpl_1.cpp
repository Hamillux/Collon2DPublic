...
#include "SceneObject/Component/EmitterComponent.h"
...
SceneType TestSceneImpl_1::Tick(float delta_seconds)
{
				...
				for (int y = 0; y < NUM_Y; y++)
				{
					for (int x = 0; x < NUM_X; x++)
					{
						...
						try
						{
							...
							{
								DxLib::printfDx("selected icon: %d\n", static_cast<int>(id));
							}
							ImGui::PopStyleVar();
							ImGui::SameLine();
						}
	...
	transform_pos = Vector2D::ViewportToWorld(DeviceInput::GetMousePosition(), screen_params);
...
void TestSceneImpl_1::UpdateScreenParams(const float delta_seconds)
{
	_screen_params.world_offset += DeviceInput::GetInputDir_WASD() * 128.f * delta_seconds / _screen_params.screen_scale;
	_screen_params.screen_scale += DeviceInput::GetMouseWheelMove() * 0.25f;
	_screen_params.screen_scale = clamp(_screen_params.screen_scale, 0.25f, 3.f);
	if (DeviceInput::IsPressed(MOUSE_INPUT_MIDDLE))
	{
		_screen_params.screen_scale = 1.f;
	}
}
