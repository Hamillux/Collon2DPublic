...
#include "SceneObject/Component/Renderer/RendererComponent.h"
...
SceneType TestSceneImpl_4::Tick(float delta_seconds)
{
	...
	CollisionManager::GetInstance().SingleLineTrace(result, query_params);
	{
		...
		const Vector2D start_screen = Vector2D::WorldToViewport(start, _screen_params);
		const Vector2D end_screen = Vector2D::WorldToViewport(end, _screen_params);
		DrawLine(start_screen.x, start_screen.y, end_screen.x, end_screen.y, GetColor(255, 0, 0));
		if (result.has_hit)
		{
			const Vector2D hit_screen = Vector2D::WorldToViewport(result.hit_location, _screen_params);
			DrawCircle(hit_screen.x, hit_screen.y, 10, GetColor(0, 255, 0));

			const Vector2D normal_end = Vector2D::WorldToViewport(result.hit_location + result.hit_normal * 50, _screen_params);
...
void TestSceneImpl_4::Draw()
{
	...
	_actor_collider->DrawDebugLines(_screen_params);
	_ground_collider->DrawDebugLines(_screen_params);
