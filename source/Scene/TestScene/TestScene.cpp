...
void TestScene::UpdateScreenParams(const float delta_seconds)
{
	_test_scene_impl->UpdateScreenParams(delta_seconds);
}
...
void TestScene::CreateTestSceneImpl(const int test_id)
{
	switch (test_id)
	{
		...
		SWITCH_CASE(3);
		SWITCH_CASE(4);
		// TODO: TestSceneImpl_NÇí«â¡ÇµÇΩèÍçáÅAÇ±Ç±Ç…í«ãL
	default:
