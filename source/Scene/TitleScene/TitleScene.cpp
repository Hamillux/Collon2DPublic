...
void TitleScene::Draw()
{
	...
	{
		constexpr const TCHAR* TEXT_TITLE = _T(u8"コロンのアクション");
		constexpr uint8_t TITLE_FONT_SIZE = 50;
		const int title_width = DrawStringHelper::GetDrawStringWidth(TITLE_FONT_SIZE, TEXT_TITLE, strlenDx(TEXT_TITLE));
		const int x = WINDOW_SIZE_X / 2 - title_width / 2;
		DrawStringHelper::DrawString(TITLE_FONT_SIZE, x, 100, TEXT_TITLE, 0xFFFFFF);

		SetFontSize(32);
		constexpr const TCHAR* TEXT_PRESS_ANY_KEY = _T(u8"Press Enter");
		int str_width = GetDrawStringWidth(TEXT_PRESS_ANY_KEY, strlenDx(TEXT_PRESS_ANY_KEY));
		DrawString(WINDOW_SIZE_X / 2 - str_width / 2, WINDOW_SIZE_Y - 128, TEXT_PRESS_ANY_KEY, 0xFFFFFF);
...
void TitleScene::UpdateScreenParams(const float delta_second)
{
	_screen_params.world_offset = _screen_params.world_offset + Vector2D(WINDOW_SIZE_X, 0) * delta_second;
}
