#include "Player.h"
#include "SceneObject/Component/Renderer/Animator/PlayerAnimatorComponent.h"
#include "PlayerAnimState.h"
#include "PlayerStatesInclude.h"
#include "SceneObject/Component/CharacterMovementComponent.h"
...
void Player::Draw(const ScreenParams& screen_params)
{
	__super::Draw(screen_params);
	_state_stack->Draw(*this, screen_params);
}
...
void Player::DrawPlayerUI(const CanvasInfo& canvas_info)
{
	...
	// コイン
	{
		...
		DrawStringHelper::DrawStringC(
			...
			std::to_string(_num_coins).c_str(),
			0xFFFFFF);
	}
}
...
void Player::DrawItemIcon(const CanvasInfo& canvas_info) const
{
	...
	// 所持数
	DxLib::DrawFormatString(ITEM_BOX_LEFT + ITEM_BOX_SIZE - 10, ITEM_BOX_TOP + ITEM_BOX_SIZE - 10, 0xFFFFFF, "x%d", item_inventory.at(current_item_type).count);
}
...
void Player::DrawHPAndSPBars(const CanvasInfo& canvas_info) const
{
	// HPバー
	{
		...
		// (現在HP) / (MAXHP)
		const std::string hp_str = std::to_string(GetHP()) + " / " + std::to_string(GetMaxHP());
		constexpr int str_left = BAR_LEFT + BAR_LENGTH + 10;
		constexpr int str_top = BAR_TOP_HP + BAR_HEIGHT / 2 - FONT_SIZE / 2;
		DrawString(str_left, str_top, _T(hp_str.c_str()), 0xFFFFFF);
	}
	...
	// SPバー
	{
		...
		// (現在SP) / (MAXSP)
		const std::string sp_str = std::to_string(_sp) + " / " + std::to_string(_max_sp);
		constexpr int str_left = BAR_LEFT + BAR_LENGTH + 10;
		constexpr int str_top = BAR_TOP_SP + BAR_HEIGHT / 2 - FONT_SIZE / 2;
		DrawString(str_left, str_top, _T(sp_str.c_str()), 0xFFFFFF);
	}
}
