...
#include "SceneObject/Component/Renderer/Animator/FlyingEnemyAnimatorComponent.h"
...
namespace
{
	constexpr float DETINATION_UPDATE_INTERVAL = 0.5f;
}
...
void FlyingEnemy::TickEnemy(const float delta_seconds)
{
	...
	else
	{
		...
		ImGui::Begin("DebugFlyingEnemy");
		ImGui::Text("CurrentTime: %.2f", GetScene()->GetWorldTime());
		ImGui::Text("DestinationUpdateTime: %.2f", _destination_update_time);
		ImGui::End();
