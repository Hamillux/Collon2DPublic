...
void RectangleBlock::Initialize(const ActorInitialParams* actor_params)
{
	...
	const float collider_local_x = _tiles_x % 2 == 0 ? UNIT_TILE_SIZE * 0.5f : 0;
	const float collider_local_y = _tiles_y % 2 == 0 ? UNIT_TILE_SIZE * 0.5f : 0;
	collider->SetLocalPosition(Vector2D{ collider_local_x, collider_local_y });
...
void RectangleBlock::GetOccupyingTileCounts(int& out_tile_x, int& out_tile_y, Vector2D& out_snap_position_to_actor_position) const
{
	out_tile_x = _tiles_x;
	out_tile_y = _tiles_y;
...
void RectangleBlock::CreateVertices(std::vector<Vertex>& out_vertices, std::vector<UINT>& out_indices)
{
	// 隣接タイルの形状を取得する関数(上下)
...
void RectangleBlock::GetVerticesOffset(float& x_offset, float& y_offset) const
{
	x_offset = static_cast<float>((UNIT_TILE_SIZE / 2) + UNIT_TILE_SIZE * ((_tiles_x- 1) / 2)) * -1.f;
	y_offset = static_cast<float>((UNIT_TILE_SIZE / 2) + UNIT_TILE_SIZE * ((_tiles_y - 1) / 2)) * -1.f;
}
