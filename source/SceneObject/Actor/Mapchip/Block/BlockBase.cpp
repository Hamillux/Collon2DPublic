...
void BlockBase::Initialize(const ActorInitialParams* actor_params)
{
	...
	is_horizontal_flip_enabled = block_params->is_horizontal_flip_enabled;

	// 頂点とインデックスの作成
	_vertices = std::vector<Vertex>();
	_indices = std::vector<UINT>();
	CreateVertices(_vertices, _indices);

	// 全頂点位置をオフセット
	{
		float x_offset = 0, y_offset = 0;
		GetVerticesOffset(x_offset, y_offset);
...
void BlockBase::Draw(const ScreenParams& screen_params)
{
	...
	// VSの設定
	{
		...
		flip_matrix._00 = is_horizontal_flip_enabled ? -1.f : 1.f;

		CB0_VS cb0_vs = {};

		cb0_vs.T_Local_to_World = (GetActorWorldTransform().ToMatrix3x3() * flip_matrix).ToXMMATRIX();
		screen_params.GetMatrixWorldToNormalizedDevice(cb0_vs.T_World_to_NDC);
...
void BlockBase::GetVerticesOffset(float& x_offset, float& y_offset) const
{
	x_offset = 0.f;
	y_offset = 0.f;
}