...
void AnimRendererComponent::RendererDraw(const ScreenParams& screen_params)
{
	if (!_is_playing)
	{
		return;
	...
	auto draw_impl = [this, &screen_params](std::unique_ptr<AnimationState>& target_anim_state)
		{
			...
			const Vector2D play_position = Vector2D::WorldToViewport(GetWorldPosition(), screen_params);
			...
			DxLib::DrawRotaGraphF(
				...
				target_anim_state->anim_info.ex_rate / screen_params.screen_scale * 1.25,	// 1.25 = ビューポートサイズ/スクリーンサイズ
				play_rotation,
				graph_handle,
				TRUE,
...
void InAnimateRenderer::RendererDraw(const ScreenParams& screen_params)
{
	if (_handle == -1)
	{
		return;
	...
	const Vector2D screen_pos = Vector2D::WorldToViewport(GetWorldPosition(), screen_params);
	const float actual_ex_rate = 1.f / screen_params.screen_scale * 1.25 * _ex_rate;
