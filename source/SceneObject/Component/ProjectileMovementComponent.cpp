...
void ProjectileMovementComponent::Tick(const float delta_seconds)
{
    ...
    ImGui::Text("CurrentVelocity:%f", GetVelocity().x, GetVelocity().y);
}
...
void ProjectileMovementComponent::EnableHoming(const Actor* const in_homing_target)
{
...
void ProjectileMovementComponent::UpdateVelocity(const float delta_seconds)
{
    ...
    if (is_homing_enabled && homing_target != nullptr)
    {
