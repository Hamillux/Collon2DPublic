...
{
    base_center_to_vertexes = std::array<Vector2D, 4>();
}
...
void BoxCollider::RespondToSingleLineTrace(QueryResult_SingleLineTrace& query_result, const CollisionQueryParams_SingleLineTrace& query_params)
{
    ...
    const FRect rect = FRect(GetWorldPosition(), base_box_diagonal.x, base_box_diagonal.y, GetWorldRotation());
...
void BoxCollider::RespondToMultiAARectTrace(QueryResult_MultiAARectTrace& query_result, const CollisionQueryParams_RectAA& query_params)
{
	...
	const FRect this_rect = FRect(GetWorldPosition(), base_box_diagonal.x, base_box_diagonal.y, GetWorldRotation());
...
void BoxCollider::GetVertexPositions(std::vector<Vector2D>& out_vertex_positions) const
{
    ...
    CalcCenterToAllVertexes(center_to_vertexes);
...
void BoxCollider::DrawDebugLines(const ScreenParams& screen_params, const ColliderDebugDrawDesc& desc)
{
    __super::DrawDebugLines(screen_params, desc);
...
{
    base_box_diagonal = new_box_extent;
    ...
    base_center_to_vertexes.at(static_cast<uint8_t>(BoxColliderVertex::LEFT_TOP)) = to_left_top;
    base_center_to_vertexes.at(static_cast<uint8_t>(BoxColliderVertex::LEFT_BOTTOM)) = to_left_bottom;
    base_center_to_vertexes.at(static_cast<uint8_t>(BoxColliderVertex::RIGHT_BOTTOM)) = to_right_bottom;
    base_center_to_vertexes.at(static_cast<uint8_t>(BoxColliderVertex::RIGHT_TOP)) = to_right_top;
}

Vector2D BoxCollider::CalcBoxDiagonal() const
{
    return Vector2D::Rotate(base_box_diagonal, GetWorldRotation());
}

Vector2D BoxCollider::CalcCenterToVertex(BoxColliderVertex vertex) const
{
    const uint8_t index = static_cast<uint8_t>(vertex);
    return Vector2D::Rotate(base_center_to_vertexes.at(index), GetWorldRotation());
}
...
FRectAA BoxCollider::GetGeometricRectAA() const
{
    ...
    FRectAA ret{};
	ret.left_top = world_pos - base_box_diagonal / 2.0f;
	ret.right_bottom = world_pos + base_box_diagonal / 2.0f;
	return ret;
}
...
void BoxCollider::CalcCenterToAllVertexes(std::array<Vector2D,4>& out_center_to_vertexes) const
{
    for (uint8_t i = 0; i < base_center_to_vertexes.size(); i++)
    {
