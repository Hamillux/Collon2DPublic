...
void SegmentCollider::DrawDebugLines(const ScreenParams& screen_params, const ColliderDebugDrawDesc& desc)
{
	...
	int x1, y1, x2, y2;
	Vector2D::WorldToViewport(left_end, screen_params).ToInt(x1, y1);
	Vector2D::WorldToViewport(right_end, screen_params).ToInt(x2, y2);
...
void SegmentCollider::DrawLine(const ScreenParams& screen_params, const int color, const int thickness)
{
	const Vector2D from = CalcSegmentEndPosition(SegmentEnd::LEFT);
	const Vector2D to = CalcSegmentEndPosition(SegmentEnd::RIGHT);
	int x1, y1, x2, y2;
	Vector2D::WorldToViewport(from, screen_params).ToInt(x1, y1);
	Vector2D::WorldToViewport(to, screen_params).ToInt(x2, y2);
