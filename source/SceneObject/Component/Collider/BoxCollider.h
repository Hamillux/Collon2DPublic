...
/*
	 現時点ではHitResult構造体から衝突したコリジョンのCollisionObjectTypeが得られないので
	 オブジェクトが複数の種類のコリジョンを持っていても、メインコリジョンのCollisionObjectTypeで判定される
*/

/**
* CalcCenterVertex()に渡して中心から頂点へのベクトルを得るために使用する
*/
...
public:
	...
	/**
	* 回転したコリジョンの対角線ベクトルの計算
	*/
	Vector2D CalcBoxDiagonal() const;
	...
	*/
	...
	Vector2D GetBoxExtent() const { return base_box_diagonal; }
...
private:
	...
	*/
	void CalcCenterToAllVertexes(std::array<Vector2D,4>& out_center_to_vertexes) const;

	// rotationがゼロのときの対角線を表すベクトル
	// 向きは左上から右下
	Vector2D base_box_diagonal;

	// rotationがゼロのときの
	// 中心から各頂点へのベクトル
	// 順番は左上から反時計回り
	std::array<Vector2D, 4> base_center_to_vertexes;
