...
{
	const int ghandle = DxLib::LoadGraph(_T(file_path.c_str()));
	if (ghandle == -1)
	{
...
{
	// 既にロードされているか確認
	if (_loaded_textures.find(image_id) != _loaded_textures.end()) {
		if (out_pp_resource)
			*out_pp_resource = _loaded_textures[image_id].p_resource.Get();
		if (out_pp_srv)
			*out_pp_srv = _loaded_textures[image_id].p_srv.Get();
...
void GraphicResourceManager::LoadSprite(const MasterDataID sprite_id)
{
	...
	DxLib::LoadDivGraph(_T(image.path.c_str()), handles.size(), num_x, num_y, size_x, size_y, handles.data());
}
...
{
	if (_loaded_graphs_for_dxlib.find(file_path) == _loaded_graphs_for_dxlib.end())
	{
		return LoadGraphForDxLib(file_path);
	}
...
{
	if (_loaded_sprites.find(sprite_id) == _loaded_sprites.end())
	{
		LoadSprite(sprite_id);
	}
