...
bool ParticleManagerImpl::Init()
{
	...
	// シェーダーのコンパイルと生成
	{
...
#ifdef _DEBUG
		if (!CompileShader(VERTEX_SHADER_PATH, _T("main"), _T("vs_5_0"), blob.GetAddressOf()))
		{
			return false;
		}
#else
...
#ifdef _DEBUG
		if (!CompileShader(PIXEL_SHADER_PATH, _T("main"), _T("ps_5_0"), blob.GetAddressOf()))
		{
			return false;
		}
#else
...
#ifdef _DEBUG
		if (!CompileShader(CS_Update_PATH, _T("main"), _T("cs_5_0"), blob.GetAddressOf()))
		{
			return false;
		}
#else
		...
		m_pCS_Spawn.Reset();
#ifdef _DEBUG
		if (!CompileShader(CS_Spawn_PATH, _T("main"), _T("cs_5_0"), blob.GetAddressOf()))
		{
			return false;
		}
#else
...
#ifdef _DEBUG
		if (!CompileShader(CS_DeactivateAll_PATH, _T("main"), _T("cs_5_0"), blob.GetAddressOf()))
		{
			return false;
		}
#else
	...
	// ピクセルシェーダー用定数バッファの作成
	{
		...
		DirectX::XMFLOAT3 rgb = DirectX::XMFLOAT3(1.f, 0.4f, 1.f);	// 塗りつぶし色
		D3D11_SUBRESOURCE_DATA initData = {};
		initData.pSysMem = &rgb;

		pDev->CreateBuffer(&desc, &initData, tempBuff.GetAddressOf());
		m_pContextRef->PSSetConstantBuffers(0, 1, tempBuff.GetAddressOf());
	}
...
void ParticleManagerImpl::Draw(const ScreenParams& screen_params)
{
	DxLib::RefreshDxLibDirect3DSetting();

	m_lastScreenParams = screen_params;
	...
	{
		CB0_VS cb0 = {};
		screen_params.GetMatrixWorldToNormalizedDevice(cb0.TransformMatrix);
...
bool ParticleManagerImpl::InitParticleBuffer(ID3D11Device* pDev)
{
	...
	// IDバッファの生成
	{
		...
		hr = pDev->CreateBuffer(&desc, &initData, m_pParticleIDBuffer.GetAddressOf());	// TODO: initDataを渡す
		if (FAILED(hr))
		{
			return false;
		}
