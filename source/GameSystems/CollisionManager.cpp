...
void CollisionManager::DrawPartition(const KdNode* node, const ScreenParams& screen_params, const int color) const
{
	...
	const Vector2D line_from_in_screen(Vector2D::WorldToViewport(line_from_in_world, screen_params));
	const Vector2D line_to_in_screen(Vector2D::WorldToViewport(line_to_in_world, screen_params));

	DrawLine(line_from_in_screen.x, line_from_in_screen.y, line_to_in_screen.x, line_to_in_screen.y, color);

	auto To2CharString = [](uint8_t x)->std::string
	{
		if (x >= 100)
		{
			return "XX";
	...
	const std::string smaller_node_index_str(To2CharString(node->GetLeftChildIndex()));
	const std::string larger_node_index_str(To2CharString(node->GetRightChildIndex()));
	const std::string partition_label = "(" + smaller_node_index_str + "," + larger_node_index_str + ")";
	constexpr uint8_t FONT_SIZE = 16;
	if (node->split_axis == Axis::X)
	{
		const int label_width = DrawStringHelper::GetDrawStringWidth(FONT_SIZE, _T(partition_label.c_str()), strlenDx(_T(partition_label.c_str())));
		Vector2D label_left_top(line_to_in_screen + Vector2D(-label_width / 2, 0));
		DrawString(label_left_top.x, label_left_top.y, _T(partition_label.c_str()), color);
	}
	else if (node->split_axis == Axis::Y)
	{
...
void CollisionManager::SplitKdNodeRecursively(KdNode* parent_node)
{
	...
	const int i_middle_collider = colliders_in_parent.size() / 2;
...
void CollisionManager::DrawCells(const ScreenParams& screen_params, const uint8_t node_depth) const
{
	if (node_depth > MAX_TREE_DEPTH)
	{
		return;
	}
	const int line_thickness = ceil(screen_params.screen_scale);
	...
	if (_root_node)
	{
		...
		(Vector2D::WorldToViewport(_root_node->cell_left_top, screen_params)).ToInt(cell_left, cell_top);
		(Vector2D::WorldToViewport(_root_node->cell_right_bottom, screen_params)).ToInt(cell_right, cell_bottom);
		DrawBoxAA(cell_left, cell_top, cell_right, cell_bottom, color, false, line_thickness);
		
		DrawCircle(cell_left, cell_top, 3, color, true);
		const std::string text_left_top = "RootNode-LeftTop";
		const int draw_str_left = cell_left - GetDrawStringWidth(_T(text_left_top.c_str()), strlenDx(_T(text_left_top.c_str()))) / 2;
		const int draw_str_top = cell_top - GetFontSize();
		DrawString(draw_str_left, draw_str_top, _T(text_left_top.c_str()), color);

		const std::string text_right_bottom = "RootNode-RightBottom";
		const int draw_str_right = cell_right - GetDrawStringWidth(_T(text_right_bottom.c_str()), strlenDx(_T(text_right_bottom.c_str()))) / 2;
		const int draw_str_bottom = cell_bottom + GetFontSize();
		DrawString(draw_str_right, draw_str_bottom, _T(text_right_bottom.c_str()), color);
	}
	...
	for (const auto& node : kd_nodes)
	{
		if(node)
		{
			DrawPartition(node, screen_params, color);
		}
	}
	...
	for (int i = 0; i <= right_most_node_number; i++)
	{
		...
		if (node)
		{
			...
			(Vector2D::WorldToViewport(node->cell_left_top, screen_params)).ToInt(x1, y1);
			(Vector2D::WorldToViewport(node->cell_right_bottom, screen_params)).ToInt(x2, y2);
